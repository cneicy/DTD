...
onEvent('block.right_click', event => {
	if (event.block == 'minecraft:enchanting_table') {
		if (!event.player.stages.has('three')) {
			event.server.runCommand(`say 你需要击败法老并将它的神器放入背包才能使用附魔台。`)
...
onEvent('item.entity_interact', event => {
    if (event.target.type == 'minecraft:ender_dragon' && event.player.getHeldItem(event.hand) == 'minecraft:bucket' && event.player.mainHandItem == 'minecraft:bucket') {
