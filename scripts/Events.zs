...
CTEventManager.register<GameStageAdded>((event) => {
    if (event.stage == "killthedragon") {
        ...
        serverworld.server.executeCommand("give @p betterendforge:guidebook", true);
    }
});
...
CTEventManager.register<MCPlayerLoggedInEvent>((event) => {
        ...
        var ban as string[] = ["projecte","slashblade","bacterium","avaritia","morph","bmorph","corail_recycler"];
